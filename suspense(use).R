...
Active <- read_excel("Global Unallocated.Active.04.04.2023.xlsx")
Allocated <- read_excel("Global Unallocated.Allocated.02.04.2023.xlsx")
Refund <- read_excel("Global Unallocated.Refunded.02.04.2023.xlsx")
...
# ----- Cumulative Suspense per Channel ---------------------------------------
...
MoMo <- Active %>% 
  filter(Name == "AIRTEL TIGO MOBILE MONEY" | 
           ...
           Name == "MOBILE MONEY - EMERGENT PAY")
...
Worksite <- Active[!(is.na(Active$Name) 
                     ...
                     | Active$Name=="MOBILE MONEY - EMERGENT PAY"), ]
...
Add.on <- Active[Active$created_on >= "2023-03-26" , ] # Plus seven(7) days
...
Worksite.Add.on <- Add.on[!(is.na(Add.on$Name) 
                            ...
                            | Add.on$Name == "MOBILE MONEY - EMERGENT PAY"), ]
